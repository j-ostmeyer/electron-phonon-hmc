...
rm -f summary_zero.csv

for dir in L_*_Nt_*_mu0_1_*_hop_*/; do
	...
	echo "$L	$Nt	-0.25	$beta	$hop	$theta	$x_avg	$n_avg	$cond	$acc	$boltz	$x2_avg	$mob" >> summary_zero.csv
	echo "$L	$Nt	0	$beta	$hop	$theta	$x_avg	$n_avg	$cond	$acc	$boltz	$x2_avg	$mob" >> summary_zero.csv
done
